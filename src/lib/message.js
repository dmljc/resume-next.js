...
export function showMessage(text, duration = 2000, options = {}) {
    try {
        ...
        const anchorEl =
            options.anchorEl ||
            (options.anchorSelector ? document.querySelector(options.anchorSelector) : null);
        ...
        if (isAnchored) {
            ...
            const offsetPx =
                typeof options.offsetPx === 'number'
                    ? options.offsetPx
                    : (options.offsetRem ?? 2) * 16;
            wrapper.style.top = `${Math.max(rect.top - offsetPx, 8)}px`;
        } else {
            wrapper.className =
                'fixed bottom-24 sm:bottom-20 md:bottom-16 left-1/2 -translate-x-1/2 z-[1000] pointer-events-none';
